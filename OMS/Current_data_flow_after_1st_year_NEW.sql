DECLARE
  ...
  YM_END   NUMBER:=0;
BEGIN
  YM_START:=DWSEAI01.YMM2YM({YMM}-5);
  ...
  FROM
    ...
    WHERE YEARMONTH>= YM_START
    AND YEARMONTH   <YM_END
    UNION ALL
    SELECT IMC_KEY_NO,
      IMC_NO,
      YEARMONTH,
    ...
    FROM
      ...
      FROM
        ...
        FROM
          ...
          FULL OUTER JOIN
            (SELECT SN_T.*,
              SN_T.YEARMONTH AS SNAP_MO_YR_KEY_NO
            FROM DWSEAI01.OMS_TEMP_RMS_PART SN_T
            WHERE YEARMONTH+100 IN (YM_END)
            ) SN_PY
