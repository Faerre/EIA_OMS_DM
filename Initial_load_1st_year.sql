...
BEGIN
 FOR i IN 24101..24105 LOOP
  ...
  INTO DWSEAI01.OMS_TEMP_RMS_PART_2021 t
...
FROM DWSANL02.JC_SEG_WITH_FST
  ...
  OMS_OLD AS
  (/*SELECT IMC_KEY_NO,
    OMS_TEMP_RMS.YEARMONTH,
    CAST(CAST(OMS_TEMP_RMS.YEARMONTH/100 AS INTEGER)*12+mod(OMS_TEMP_RMS.YEARMONTH, 100) AS INTEGER) AS YEARMONTH_MONTHS,
    COUNTRY,
    COUNTRY_PY,
    SUBREGION,
    SALES_REGION,
    ...
    DIST_TYPE,
    DIST_TYPE_PY
  FROM DWSEAI01.OMS_TEMP_RMS_PART OMS_TEMP_RMS
  WHERE OMS_TEMP_RMS.YEARMONTH>=
    $${IMAGO}$$-
    CASE
      WHEN MOD(
        $${IMAGO}$$, 100)>3
